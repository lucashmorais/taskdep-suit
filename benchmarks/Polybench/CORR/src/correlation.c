  ...
  for (i=1; i < (M+1); i++)
      ...
      for (j=1; j < (N+1); j++)
	{
	  if (percentDiff(symmat[i*(N+1) + j], symmat_outputFromGpu[i*(N+1) + j]) > ERROR_THRESHOLD)
	    {
	      fail++;
	      //printf("i: %d j: %d\n1: %f 2: %f\n", i, j, symmat[i*N + j], symmat_FPGA[i*N + j]);		
	    }
	}
    }
...
int main()
{
  ...
  t_start = rtclock();
  correlation_OMP(data, mean, stddev, symmat_FPGA);
  t_end = rtclock();
  ...
  compareResults(symmat, symmat_FPGA);
