 ...
 */
void RayEngine::run_seq() {
    for(int i=0; i<yres; i++) {
        render_scanline_seq(xres, yres, i, out, rays_per_pixel);
    }
}
...
void RayEngine::render_scanline_seq(int xsz, int ysz, int sl, Image& out, int samples) {
        int i, s;
        double rcp_samples = 1.0 / (double)samples;
        Pixel p;
