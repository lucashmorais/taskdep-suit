...
// *   ------------------------
// *   Wrapper function for computeRow to expose inputs and outputs to the OmpSs runtime.
// */
// void computeRowTask(int row, int rowwidth, float xot, float yot, float xos, float yos, unsigned rev_angle, RotateEngine* re) {
//     re->computeRow(row, rowwidth, xot, yot, xos, yos, rev_angle);
// }
...
void RotateEngine::run() {
	...
	*/
    ...
    for(int i = 0; i < target_h; i+=4) {
		Pixel* out_ptr = &this->getOutput().getPixels()[width*i];

        #pragma omp task depend (out: out_ptr) firstprivate(i, target_h)
