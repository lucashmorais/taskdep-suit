...
float pspeedy_seq(Points *points, float z, long *kcenter)
{
...
// #pragma omp task depend(in: points[bsize])
...
float pspeedy(Points *points, float z, long *kcenter)
{
    ...
    #pragma omp single
    {
    for(int i = 0; i < evenpoints; i += PSPEEDY_BSIZE) {
        Point* p_t = &(points->p[i]);

        #pragma omp task firstprivate(i) depend(in: p_t[0:PSPEEDY_BSIZE])
        pspeedy_task_initial(&(points->p[i]), points->p, PSPEEDY_BSIZE, points->dim);
    }

    if(remainder != 0) {
        Point* p_t = &(points->p[evenpoints]);

        #pragma omp task depend(in: p_t[0:remainder])
...
*         if switched
*/
// #pragma omp task depend(in: p[bsize])
...
double pgain_seq(long x, Points *points, double z, long int *numcenters)
{
    int i;
...
double pgain(long x, Points *points, double z, long int *numcenters)
{
    ...
    */
    ...
    if(remainder != 0) {
        ...
        #pragma omp task firstprivate(tasks, remainder) \
         depend(in: p_t[0:remainder])
 ...
 float pFL_seq(Points *points, int *feasible, int numfeasible,
      float z, long *k, double cost, long iter, float e)
{
    ...
    while (change/cost > 1.0*e) {
        ...
        for (i=0;i<iter;i++) {
            x = i%numfeasible;
            change += pgain_seq(feasible[x], points, z, k);
        }
        cost -= change;
    }
    return(cost);
...
// #pragma omp task depend(in: *ref, p[0:bsize]) depend(out: *hiz)
...
float pkmedian_seq(Points *points, long kmin, long kmax, long* kfinal)
{
    int i;
    double cost;
    double lastcost;
    ...
    while(1) {
        ...
        lastcost = cost;
        cost = pFL_seq(points, feasible, numfeasible,
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal)
{
    ...
    #pragma omp single
    {
    for(int i = 0; i < evenpoints; i += PKMEDIAN_BSIZE) {
        Point* p_t = &(points->p[i]);
        Point* ref = &points->p[0];

        #pragma omp task firstprivate(i, ptDimension) \
         depend(in: ref, p_t[0:PKMEDIAN_BSIZE]) \
         depend(out: hiz_tmp[count])
    ...
    if(remainder != 0) {
        ...
        #pragma omp task firstprivate(i, remainder, ptDimension) \
         depend(in: ref, p_t[0:remainder]) \
         depend(out: hiz_tmp[count])
...
void localSearch_seq( Points* points, long kmin, long kmax, long* kfinal ) {
    pkmedian_seq(points, kmin, kmax, kfinal);
}
...
void streamCluster_seq( SimStream* stream,
            long kmin, long kmax, int dim,
            long chunksize, long centersize, char* outfile ) {
    ...
    while(1) {
        ...
        is_center = (int*)calloc(points.num,sizeof(int));
        center_table = (int*)malloc(points.num*sizeof(int));
        localSearch_seq(&points, kmin, kmax, &kfinal); // parallel
    ...
    localSearch_seq( &centers, kmin, kmax ,&kfinal ); // parallel
    contcenters(&centers);
    outcenterIDs( &centers, centerIDs, outfile);
...
int main(int argc, char **argv)
{
    ...
    int dim;
    int parallel;
    timer start, end, startseq, endseq;

    fprintf(stderr,"Streamcluster Kernel\n");
    fflush(NULL);

    if (argc<9) {
        fprintf(stderr,"usage: %s k1 k2 d n chunksize clustersize infile outfile parallel\n",
            argv[0]);
        ...
        fprintf(stderr,"  parallel:     0 for sequential, 1 for parallel\n");
        fprintf(stderr,"\n");
    ...
    if (parallel < 0 || parallel > 1) {
        printf("Parallel flag must be 0 or 1\n");
        exit(-1);
    }

    srand48(SEED);
	SimStream stream, streamseq;
	if(n > 0) {
		stream.n = n;
                      streamseq.n = n;
	} else {
    ...
    printf("Starting parallel code...\n");
        ...
        streamCluster_seq(&streamseq, kmin, kmax, dim, chunksize, clustersize, outfilename );

        TIME(endseq);
        double ctime_msec_seq = (double)timevaldiff(&startseq, &endseq);
        printf("Time: %lf\n", (ctime_msec_seq/1000));
    }
