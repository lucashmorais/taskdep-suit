...
/*
Explanation of constants and variables used in this function:s
...
{
    double x_coord = 0.0, y_coord = 0.0, pi, r = 0.0;
    int score, n;
    ...
    for (n = 1; n <= darts; n++)  {
// #pragma omp task label( throw_dart ) firstprivate( n ) in( cconst ) reduction( +:score ) no_copy_deps
        {
        /* generate random numbers for x and y coordinates */
        r = (double)rand_r(&myseed)/cconst;
        x_coord = (2.0 * r) - 1.0;
        r = (double)rand_r(&myseed)/cconst;
        y_coord = (2.0 * r) - 1.0;
