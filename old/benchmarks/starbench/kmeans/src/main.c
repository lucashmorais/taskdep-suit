...
int main(int argc, char **argv) {
    ...
    double   threshold;
    double  timing, io_timing, clustering_timing, timingseq, io_timingseq, clustering_timingseq;
    ...
    while ( (opt=getopt(argc,argv,"o:p:i:n:t:absd"))!= EOF) {
        switch (opt) {
...
#pragma omp single
	...
	io_timing += seconds(t1) - timing;
        ...
        seconds(timingseq);
        clustering_timingseq = timingseq;

        membership = (int*) malloc(numObjs * sizeof(int));
        assert(membership != NULL);

        /* Launch the core computation algorithm */
        clusters = kmeans_seq(0, objects, numCoords, numObjs,
                              numClusters, threshold, membership);

        free(objects[0]);
        free(objects);
        seconds(timingseq);
        clustering_timingseq = timingseq - clustering_timingseq;
    ...
    /*FILE *file = fopen("../output/par", "w");
    fprintf(file, "numObjs       = %d\n", numObjs);
    fprintf(file, "numCoords     = %d\n", numCoords);
    fprintf(file, "numClusters   = %d\n", numClusters);
    fprintf(file, "threshold     = %.4f\n", threshold);
    fclose(file);*/

    printf("I/O time           = %10.8f sec\n", io_timing);
    if (parallel)
        printf("Time: %10.8f\n", clustering_timing);
    else
        printf("Time: %10.8f\n", clustering_timingseq);
