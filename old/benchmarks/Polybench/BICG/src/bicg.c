...
void bicg_OMP(DATA_TYPE* A, DATA_TYPE* r, DATA_TYPE* s, DATA_TYPE* p, DATA_TYPE* q)
{
  ...
  #pragma omp target device (FPGA_DEVICE) map(to: A[:NX*NY], p[:NY], r[:NX]) map(tofrom: s[:NY], q[:NX])
  {
...
int main(int argc, char** argv)
{
  ...
  s_FPGA = (DATA_TYPE*)malloc(NY*sizeof(DATA_TYPE));
  q_FPGA = (DATA_TYPE*)malloc(NX*sizeof(DATA_TYPE));
  ...
  compareResults(s, s_FPGA, q, q_FPGA);
