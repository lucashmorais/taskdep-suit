/*-----------------------------------------------------------------------*/
/* Program: Stream                                                       */
/* Adapted back to openMP 4.5 by Vitor Silva (IME-USP)                   */
/*-----------------------------------------------------------------------*/
...
/* Revision: $Id: stream.c,v 5.8 2007/02/19 23:57:39                     */
/*												mccalpin Exp mccalpin $  */
...
void
init_task(double *a, double *b, double *c, int bs) {
	int j;	
	for (j=0; j < BSIZE; j++){
	        a[j] = 1.0;
	        b[j] = 2.0;
...
tuned_initialization() {
	int j;
        for (j=0; j<N; j+=BSIZE)
			...
			#pragma omp task depend(out: a[j:BSIZE], b[j:BSIZE], c[j:BSIZE])
...
//Assumes N is multiple of 100
			#pragma omp task depend(in: a[j:BSIZE]) depend(out: c[j:BSIZE])
...
//Assumes N is multiple of 100 
	for (j=0; j<N; j+=BSIZE)
			#pragma omp task depend(in: c[j:BSIZE]) depend(out: b[j:BSIZE])
...
//Assumes N is multiple of 100 
	for (j=0; j<N; j+=BSIZE)
		#pragma omp task depend(in: a[j:BSIZE], b[j:BSIZE]) depend(out: c[j:BSIZE])
...
//Assumes N is multiple of 100 
	for (j=0; j<N; j+=BSIZE)
		#pragma omp task depend(in: b[j:BSIZE], c[j:BSIZE]) depend(out: a[j:BSIZE])
