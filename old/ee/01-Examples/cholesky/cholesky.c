...
void omp_potrf(double * const A, const MKL_INT ts, const MKL_INT ld)
{
   static MKL_INT INFO;
...
void omp_trsm(double *A, double *B, const MKL_INT ts, int ld)
{
...
void cholesky_blocked(const int ts, const int nt, double* Ah[nt][nt])
{
   for (int k = 0; k < nt; k++) {
        ...
        #pragma omp task depend(inout: Ah[ts][ts])
        // Diagonal Block factorization
        omp_potrf (Ah[k][k], ts, ts);

        // Triangular systems
        for (int i = k + 1; i < nt; i++) {
            #pragma omp task depend(in: Ah[ts][ts]) depend(inout: Ah[ts][ts])
    ...
    for (int i = k + 1; i < nt; i++) {
        for (int j = k + 1; j < i; j++) {
            #pragma omp task depend(in: Ah[ts][ts], Ah[ts][ts]) depend(inout: Ah[ts][ts])
...
int main(int argc, char* argv[])
{
   ...
   const MKL_INT n = (MKL_INT) atoi(argv[1]); // matrix size
