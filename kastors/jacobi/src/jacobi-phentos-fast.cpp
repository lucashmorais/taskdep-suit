        ...
        for (it = itold + 1; it <= itnew; it++) {
            ...
            for (i = 0; i < nx; i++) {
				...
				*/
				...
				initiate_task_or_work_fast(swID, 5, numPendingWorkRequests);
				add_args_and_parent_info_or_work_fast(0, 0, numPendingWorkRequests);
				
				//submit_v4_out_dep_or_work_fast((unsigned long long) &((*u)[i][0]), numPendingWorkRequests);
				//submit_v4_in_dep_or_work_fast((unsigned long long) &((*unew)[i][0]), numPendingWorkRequests);
				submit_v4_in_dep_or_work_fast((unsigned long long) &((*f)[i][0]), numPendingWorkRequests);
				submit_v4_in_dep_or_work_fast((unsigned long long) &((*u)[i-1][0]), numPendingWorkRequests);
				submit_v4_in_dep_or_work_fast((unsigned long long) &((*u)[i][0]), numPendingWorkRequests);
				submit_v4_in_dep_or_work_fast((unsigned long long) &((*u)[i+1][0]), numPendingWorkRequests);
				submit_v4_out_dep_or_work_fast((unsigned long long) &((*unew)[i][0]), numPendingWorkRequests);
            }
        }
